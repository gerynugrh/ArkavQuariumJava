...
public class Snail extends AquariumObject implements Collector {
    ...
    private void move(double secSinceLast) {
        ...
        if (nearestValuable.getPosition().x < position.x && position.x - nearestValuable.getPosition().x >= 20) {
            direction = -1;
        } else if (nearestValuable.getPosition().x > position.x && nearestValuable.getPosition().x - position.x >= 20) {
            direction = 1;
